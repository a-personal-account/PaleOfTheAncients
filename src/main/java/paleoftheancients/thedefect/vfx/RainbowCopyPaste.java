...
public class RainbowCopyPaste extends AbstractGameEffect {
    float x;
    float y;
    private TextureAtlas.AtlasRegion img;

    public RainbowCopyPaste(AbstractCreature ac) {
        if (this.img == null) {
            this.img = ImageMaster.CRYSTAL_IMPACT;
        }

        this.x = ac.hb.cX - (float)this.img.packedWidth / 2.0F;
        this.y = ac.hb.cY - (float)this.img.packedHeight / 2.0F;
